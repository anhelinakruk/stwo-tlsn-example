    ...
    SimdBackend: BackendForChannel<MC>,
{
    ...
    println!("DEBUG commitment lengths PROVER: {:?}", commitment_scheme.trees.len());
    // Prove constraints.
...
pub fn verify_blake<MC: MerkleChannel>(
    ...
    eprintln!("DEBUG log_sizes lengths: {:?}", log_sizes.0.iter().map(|v| v.len()).collect::<Vec<_>>());
    eprintln!("DEBUG commitment lengths: {:?}", stark_proof.commitments.len());
