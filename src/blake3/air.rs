    ...
    SimdBackend: BackendForChannel<MC>,
{
    ...
    eprintln!("=== DEBUG PROVER TRACE LAYOUT ===");
    eprintln!("log_size (input) = {}", log_size);
    eprintln!("LOG_N_LANES = {}", LOG_N_LANES);
    eprintln!("ROUND_LOG_SPLIT = {:?}", ROUND_LOG_SPLIT);
    eprintln!("N_ROUNDS = {}", N_ROUNDS);

    // If you can access commitment_scheme internals:
    eprintln!("commitment_scheme.trees.len() = {}", commitment_scheme.trees.len());
    ...
    // eprintln!("DEBUG commitment lengths PROVER: {:?}", commitment_scheme.trees.len());
    // Prove constraints.
    let components = BlakeComponents::new(&stmt0, &all_elements, &stmt1);
    let component_provers = components.component_provers();

    eprintln!("DEBUG PROVER: commitment_scheme.trees.len() = {}", commitment_scheme.trees.len());

// move
let stark_proof = prove(&component_provers, channel, commitment_scheme).unwrap();

// debug the returned proof
eprintln!("DEBUG PROVER: stark_proof.commitments.len() = {}", stark_proof.commitments.len());
for (i, c) in stark_proof.commitments.iter().enumerate() {
    eprintln!("DEBUG PROVER: commitment[{}] type = {}", i, std::any::type_name_of_val(c));
    eprintln!("DEBUG PROVER: commitment[{}] debug = {:#?}", i, c);
}
...
pub fn verify_blake<MC: MerkleChannel>(
    ...
    eprintln!("=== VERY DETAILED VERIFIER LAYOUT ===");
eprintln!("log_sizes.len() = {}", log_sizes.0.len());
for (i, v) in log_sizes.0.iter().enumerate() {
    eprintln!("  log_sizes[{}].len() = {}", i, v.len());
    eprintln!("  log_sizes[{}] = {:?}", i, v); // krótkie podglądnięcie wektorów jeśli nie za duże
}

eprintln!("stark_proof.commitments.len() = {}", stark_proof.commitments.len());
for (i, c) in stark_proof.commitments.iter().enumerate() {
    eprintln!("  commitment[{}] type = {}", i, std::any::type_name_of_val(c));
}
